...
const bookeAppointmentController = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message: "Appoointment Has Been Booking Succesfully",
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
...
// bokking avaliability controller
...
const userAppointmentController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Error In Fetching Users Appointments",
    });
  }
};
